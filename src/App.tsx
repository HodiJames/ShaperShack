...
// ─────────────────────────────────────────────
// SURFER PHOTOS GALLERY - Community board photos
// ─────────────────────────────────────────────
function SurferPhotosGallery({ listing }) {
  const [photos, setPhotos] = useState([]);
  const [loading, setLoading] = useState(true);
  const [lightbox, setLightbox] = useState(null);

  useEffect(() => {
    fetch(`${API_BASE}/api/surfer-photos/${listing.id}`)
      .then(r => r.json())
  ...
  return (
    <div className="ld-sec">
      <div className="ld-sec-title">
        📸 Surfers Using Boards By {firstName}
        <span style={{ marginLeft: 8, fontSize: 12, fontWeight: 400, color: "var(--txm)" }}>
          {photos.length} photo{photos.length !== 1 ? "s" : ""}
        </span>
      </div>
      <p style={{ fontSize: 13, color: "var(--tx2)", marginBottom: 16, lineHeight: 1.6 }}>
        Real surfers sharing their {firstName} boards. Add your own from your Quiver!
      </p>
      
      <div className="surfers-gallery">
        {photos.slice(0, 8).map((photo, i) => (
          <div key={i} className="surfer-card" onClick={() => setLightbox(photo)}>
            <img src={photo.url} alt={photo.boardName} className="surfer-card-img" />
            <div className="surfer-card-body">
              <div className="surfer-card-board">{photo.boardName}</div>
              <div className="surfer-card-user">by {photo.userName}</div>
              {photo.boardLength && (
                <div className="surfer-card-meta">
                  <span className="surfer-card-chip">📏 {photo.boardLength}</span>
                </div>
              )}
            </div>
          </div>
      ...
      {photos.length > 8 && (
        <p style={{ fontSize: 12, color: "var(--txm)", textAlign: "center", marginTop: 14 }}>
      ...
      {lightbox && (
        <div className="lightbox-overlay" onClick={() => setLightbox(null)}>
          ...
          <div onClick={e => e.stopPropagation()} style={{ textAlign: "center" }}>
            <img src={lightbox.url} alt={lightbox.boardName} className="lightbox-img" />
            <div style={{ color: "#fff", marginTop: 16 }}>
              <div style={{ fontSize: 18, fontWeight: 600 }}>{lightbox.boardName}</div>
              <div style={{ fontSize: 14, opacity: 0.8 }}>by {lightbox.userName}</div>
            </div>
          </div>
        </div>
      )}
  ...
  return (
    <div className="ldp">
      ...
      <div className="ld-body">
        ...
        {/* Surfers Using Boards By [Shaper] */}
        <SurferPhotosGallery listing={listing} />
...
function emptyBoard() {
  return {
    ...
    photos: [], // User photos of this board (max 5)
  };
}

function QuiverTab({ listings }) {
  const { setPage, user, showToast } = useContext(Ctx);
  ...
  useEffect(() => {
    ...
    const loadBoards = async () => {
      try {
        const res = await fetch(`${API_BASE}/api/quiver/${encodeURIComponent(user.email)}`);
        if (res.ok) {
          const data = await res.json();
          setBoards(data.length > 0 ? data : DEMO_QUIVER);
        }
      } catch (err) {
        console.error('Failed to load quiver:', err);
        setBoards(DEMO_QUIVER);
      }
      setLoading(false);
    };
  ...
  const syncBoard = async (board) => {
    ...
    try {
      await fetch(`${API_BASE}/api/quiver/${encodeURIComponent(user.email)}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ board: { ...board, userName: user.name || user.firstName || 'Anonymous' } })
      });
    } catch (err) {
  ...
  const saveBoard = async () => {
    ...
    const board = { ...form, id: editing || Date.now() };
    if (editing) { setBoards(p => p.map(b => b.id === editing ? board : b)); }
    else { setBoards(p => [board, ...p]); }
    await syncBoard(board);
    setAdding(false); setEditing(null);
    showToast(editing ? "Board updated!" : "Board added to quiver!");
  };

  const archive = async (id) => {
    const board = boards.find(b => b.id === id);
    if (board) {
      const updated = { ...board, archived: true };
      setBoards(p => p.map(b => b.id === id ? updated : b));
      await syncBoard(updated);
    }
  };
  ...
  const restore = async (id) => {
    const board = boards.find(b => b.id === id);
    if (board) {
      const updated = { ...board, archived: false };
      setBoards(p => p.map(b => b.id === id ? updated : b));
      await syncBoard(updated);
    }
  };
  ...
  const remove = async (id) => {
    ...
    if (user?.email) {
      try {
        await fetch(`${API_BASE}/api/quiver/${encodeURIComponent(user.email)}/${id}`, { method: 'DELETE' });
      } catch (err) { console.error(err); }
    }
  };

  const rate = async (id, n) => {
    const board = boards.find(b => b.id === id);
    if (board) {
      const updated = { ...board, rating: board.rating === n ? 0 : n };
      setBoards(p => p.map(b => b.id === id ? updated : b));
      await syncBoard(updated);
    }
  };
  ...
  if (loading) {
    return <div className="qv-wrap"><div className="qv-empty">Loading your quiver...</div></div>;
  }

  // Render board with optional inline edit form
  const renderBoard = (b, isPast = false) => (
    <div key={b.id}>
      <BoardCard 
        board={b} 
        past={isPast}
        onEdit={() => openEdit(b)} 
        onArchive={isPast ? undefined : () => archive(b.id)}
        onRestore={isPast ? () => restore(b.id) : undefined}
        onDelete={isPast ? () => remove(b.id) : undefined}
        onRate={n => rate(b.id, n)} 
        onNavigate={() => setPage("listing")} 
        listings={listings} 
      />
      {editing === b.id && (
        <div className="qv-inline-edit">
          <BoardForm 
            form={form} 
            h={h} 
            listings={listings}
  ...
  return (
    <div className="qv-wrap">
      ...
      {/* Add new board form at top when adding */}
      {adding && (
        <div className="qv-inline-edit" style={{ marginBottom: 16 }}>
          <BoardForm 
            form={form} 
            h={h} 
            listings={listings}
            title="Add a board to your quiver"
            onSave={saveBoard} 
            onCancel={cancelForm} 
            setForm={setForm} 
          />
      ...
      {currentBoards.map(b => renderBoard(b, false))}
      ...
      {pastBoards.length > 0 && (
        <>
          ...
          {pastBoards.map(b => renderBoard(b, true))}
        </>
      )}
    </div>
  );
...
function BoardCard({ board: b, past, onEdit, onArchive, onRestore, onDelete, onRate, onNavigate, listings }) {
  ...
  return (
    <div className={`qv-board ${past ? "past" : ""}`} style={{ marginBottom:12 }}>
      ...
      {b.photos?.length > 0 && (
        <div className="qv-board-photos">
          {b.photos.slice(0, 3).map((photo, i) => (
            <img key={i} src={photo.url} alt={`${b.name} photo ${i + 1}`} className="qv-board-photo" />
          ))}
          {b.photos.length > 3 && (
            <div className="qv-board-photo-more">+{b.photos.length - 3}</div>
          )}
        </div>
      )}
      <div className="qv-board-top">
        <div className="qv-board-main">
          ...
          <div className="qv-board-meta">
            ...
            {b.photos?.length > 0 && <span className="qv-chip" style={{background:"#e8f4fb",color:"#1a6080"}}>📷 {b.photos.length} photo{b.photos.length > 1 ? "s" : ""}</span>}
          </div>
        </div>
...
function BoardForm({ form, h, listings, title, onSave, onCancel, setForm }) {
  const { setPage, showToast, user } = useContext(Ctx);
  ...
  const handlePhotoUpload = async (e) => {
    ...
    if (remaining <= 0) {
      showToast("Maximum 5 photos per board");
      return;
    }
    ...
    try {
      ...
      for (let i = 0; i < filesToUpload.length; i++) {
        ...
        if (file.size > 5 * 1024 * 1024) {
          showToast(`${file.name} too large (max 5MB)`);
          continue;
        }
        
        const result = await uploadImage(file, `quiver/${user?.email || 'anon'}`, (progress) => {
          setPhotoProgress(Math.round(((i + progress / 100) / filesToUpload.length) * 100));
        });
        uploaded.push({ ...result, userName: user?.name || user?.firstName || 'Anonymous' });
      }
      
      h("photos", [...currentPhotos, ...uploaded]);
  ...
  const removePhoto = async (index) => {
    ...
    if (photo.path) {
      try { await deleteFile(photo.path); } catch (err) { console.error(err); }
    }
    h("photos", form.photos.filter((_, i) => i !== index));
  };
  ...
  return (
    <div className="qv-form">
      ...
      <div className="fg">
        <label className="fl">📷 Board Photos <span style={{fontWeight:400,color:"var(--txm)"}}>(max 5)</span></label>
        {(form.photos?.length > 0) && (
          <div className="qv-photos-grid">
            {form.photos.map((photo, i) => (
              <div key={i} className="qv-photo-item">
                <img src={photo.url} alt={`Board photo ${i + 1}`} />
                <button className="qv-photo-remove" onClick={() => removePhoto(i)}>✕</button>
              </div>
            ))}
          </div>
        )}
        ...
        {(form.photos?.length || 0) < 5 && (
          <button
            type="button"
            className="qv-photo-add-btn"
            onClick={() => photoInputRef.current?.click()}
            disabled={uploadingPhotos}
          >
            {uploadingPhotos ? `Uploading... ${photoProgress}%` : `+ Add Photos (${5 - (form.photos?.length || 0)} remaining)`}
          </button>
        )}
        <p style={{ fontSize: 11, color: "var(--txm)", marginTop: 6 }}>
          Share photos of your board! These may appear on the shaper's listing page.
        </p>
      </div>
      ...
      <div className="shell" dir={localeObj.dir}>
        ...
        <nav className="nav">
          ...
          <div className="nav-r">
            ...
            <button className="btn bp nav-submit-btn" onClick={() => setPage("submit")}>
              <span className="nav-submit-full">＋ Submit a Shaper</span>
              <span className="nav-submit-short">＋</span>
            </button>
            {user ? <>
              <button className={`nav-bm-btn ${savedIds.length>0?"has-saved":""}`} onClick={()=>setPage("saved")}>
                ...
                <span className="nav-saved-text">{tr("nav.saved")}{savedIds.length>0?` (${savedIds.length})`:""}
                </span>
              </button>
              {user.role==="admin" && (
                <button className="nav-admin-btn nav-hide-mobile" onClick={()=>setPage("admin")}>
              ...
              <button className="btn bo bsm nav-profile-btn" onClick={()=>setPage("profile")} style={{display:"flex",alignItems:"center",gap:6}}>
                ...
                <span className="nav-profile-name">{user.firstName||user.name.split(" ")[0]}</span>
              </button>
            </> : <>
              <button className="btn bg" onClick={()=>setModal("in")}>{tr("nav.signin")}</button>
              <button className="btn bo nav-hide-mobile" onClick={()=>setModal("up")}>{tr("nav.join")}</button>
            </>}
          </div>
        </nav>
